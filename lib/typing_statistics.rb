...
class TypingStatistics
  ...
  def self.statistics(scores)
    ...
    scores.each do |key, val|
      ...
      @averaged_statistics[key][0] = ((@averaged_statistics[key][0] + val[0]) / (@averaged_statistics[key][2] + val[2]))
      # Storing the average accuracy to the averaged hash
      @averaged_statistics[key][1] = ((@averaged_statistics[key][1] + val[1]) / (@averaged_statistics[key][2] + val[2]))
      # Storing the amount of times each symbol has been displayed to the averaged hash
      @averaged_statistics[key][2] = (@averaged_statistics[key][2] + val[2])
    end
